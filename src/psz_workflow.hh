...
#pragma omp parallel for
            for (size_t b0 = 0; b0 < dims_L16[nBLK0]; b0++) {
                if (fine_massive)
#ifdef REPBLK
                    PdQ::c_lorenzo_1d1l<T, Q, B>(data, outlier, code, dims_L16, ebs_L4, pred_err, comp_err, 0);
#else
                    PdQ::c_lorenzo_1d1l<T, Q, B>(data, outlier, code, dims_L16, ebs_L4, pred_err, comp_err, b0);
#endif
                else
...
#pragma omp parallel for
            for (size_t b1 = 0; b1 < dims_L16[nBLK1]; b1++) {
                for (size_t b0 = 0; b0 < dims_L16[nBLK0]; b0++) {
                    if (fine_massive)
#ifdef REPBLK
                        PdQ::c_lorenzo_2d1l<T, Q, B>(data, outlier, code, dims_L16, ebs_L4, pred_err, comp_err, 0, 0);
#else
                        PdQ::c_lorenzo_2d1l<T, Q, B>(data, outlier, code, dims_L16, ebs_L4, pred_err, comp_err, b0, b1);
#endif
                    else
            ...
            for (size_t b2 = 0; b2 < dims_L16[nBLK2]; b2++) {
                for (size_t b1 = 0; b1 < dims_L16[nBLK1]; b1++) {
                    for (size_t b0 = 0; b0 < dims_L16[nBLK0]; b0++) {
                        if (fine_massive)
#ifdef REPBLK
                            PdQ::c_lorenzo_3d1l<T, Q, B>(data, outlier, code, dims_L16, ebs_L4, pred_err, comp_err, 0, 0, 0);
#else
                            PdQ::c_lorenzo_3d1l<T, Q, B>(data, outlier, code, dims_L16, ebs_L4, pred_err, comp_err, b0, b1, b2);
#endif
                        else
...
//    }
    ...
    cout << "O3 AVG PRED/QUANT time: " << tot_cx_time/ITERS << "s" << endl;
...
/*	FILE* fp1 = fopen("codewords.csv","w");
	fprintf(fp1,"index,Length,Codeword\n");
	for (int i = 0; i < tree->stateNum; i++)
	{
		if (tree->code[i] == NULL)
		{
			fprintf(fp1,"%d,0,0\n",i);
...
#endif
/*
	FILE *fp2 = fopen("outliers.csv","w");
	fprintf(fp2,"outliers\n");

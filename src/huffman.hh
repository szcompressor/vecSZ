...
void initHuffman(HuffmanTree* huffmanTree, const int* s, size_t length) {
    ...
    */
    size_t maximal = 0,minimal = SIZE_MAX;
    for (i = 0; i < huffmanTree->allNodes; i++) {
        if (freq[i]) {
            minimal = minimal > freq[i] ? freq[i] : minimal;
            maximal = maximal < freq[i] ? freq[i] : maximal;
...
void hMD(HuffmanTree* huffmanTree, const int *s, size_t length,int *depth) {
    size_t   i;
	int state,bitSize,max=-1;
	for (i = 0; i < length; i++) {
        state   = s[i];
        bitSize = huffmanTree->cout[state];
		if (bitSize > max) max = bitSize;
	}
	*depth = max;
}

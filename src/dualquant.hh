	...
	if (id < dims_L16[DIM0] + blksz) 
  {
    ...
    if (vector_reg == 512) 
    {
		  ...
		  for (;id < blk_end16; id += 16) 
      {
    		...
    		if (id < _idx0 + 1) vpred = _mm512_maskz_loadu_ps(pMask_512,&data[id - 1]);
    		else vpred = _mm512_loadu_ps(&data[id - 1]);
    ...
    for (;id < blk_end8; id += 8)
    {
    	...
    	__m256 vpred;
    	if (id < _idx0 + 1) vpred = _mm256_maskload_ps(&data[id - 1],pMask_256);
     	else vpred = _mm256_loadu_ps(&data[id - 1]);
...
void c_lorenzo_2d1l(Data*                  data,
                    ...
                    int                 vector_reg) 
{    
  alignas(32) Data _s[blksz + 1][blksz + 1]; // 2D interpretation of data
  ...
  if (_idx1 < dims_L16[DIM1] and _idx0 < dims_L16[DIM0]) // vectorizable cases
  {
    ...
    for (size_t i1 = 0; i1 < blksz; i1++) 
    {
      ...
      if (vector_reg == 512) 
      {
	 	    for(; id < blk_end16; id += 16, i0 += 16) //AVX-512
        {
...
#endif
      for (; id < blk_end8; id += 8,i0 += 8) //AVX2
      {
...
#endif
      for (; id < blk_end8; id += 8,i0 += 8) //AVX-256
      {
...
// prequantization
  for (size_t i2 = 0; i2 < blksz; i2++) 
  {
    for (size_t i1 = 0; i1 < blksz; i1++) 
    {
      ...
      size_t gi2 = _idx2 + i2;
      size_t gi1 = _idx1 + i1;
      size_t id  = _idx0 + (_idx1 + i1) * dims_L16[DIM0] + (_idx2 + i2) * dims_L16[DIM1] * dims_L16[DIM0];
      size_t blk_end = _idx0 + (_idx1 + i1) * dims_L16[DIM0] + (_idx2 + i2) * dims_L16[DIM1] * dims_L16[DIM0] + blksz;
      size_t blk_end16 = (blk_end & ~0xF);
      size_t blk_end8  = (blk_end & ~0x7);
      ...
      for (size_t i1 = 0; i1 < blksz; i1++) 
      {
        ...
        if (vector_reg == 512) 
        {
          for(; id < blk_end16; id += 16, i0 += 16) //AVX-512
          {
            ...
            size_t gi1 = _idx1 + i1;
            size_t gi0 = _idx0 + i0 + 15;
            if (gi2 >= dims_L16[DIM2] or gi1 >= dims_L16[DIM1] or gi0 >= dims_L16[DIM0]) break;
            __m512 vdata = _mm512_loadu_ps(&data[id]);
            __m512 s = _mm512_roundscale_ps(_mm512_mul_ps(vdata,vebx2),(_MM_FROUND_TO_NEAREST_INT |_MM_FROUND_NO_EXC));
            _mm512_storeu_ps(&_s[i2+1][i1+1][i0+1],s);
          }

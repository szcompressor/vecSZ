...
void c_lorenzo_1d1l(T* data, T* outlier, Q* bcode, size_t const* const dims_L16, double const* const ebs_L4, T* pred_err, T* comp_err, size_t b0) {
    ...
    for (size_t i0 = 0; i0 < B; i0++) {
        size_t id = _idx0 + i0;
        if (id >= dims_L16[DIM0]) continue;                   //Conservative   --- Leniant
        T    pred        = id < _idx0 + 1 ? 0 : data[id - 1]; //LOAD 1         --- LOAD 1
        T    posterr     = data[id] - pred;		      //SUB 1 , LOAD 1 --- SUB 1, LOAD 1
        bool quantizable = fabs(posterr) < radius;            //ABS 1          --- ABS 1, CMP 1
        Q    _code       = static_cast<Q>(posterr + radius);  //ADD 1          --- ADD 1, CAST 1
        outlier[id]      = (1 - quantizable) * data[id];      //MUL 1, STORE 1 --- MUL 1, STORE 1

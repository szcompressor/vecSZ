...
if (vector_reg == 512) {
	for (; id < blk_end16; id += 16) //AVX-512
	{
		...
		const __m512 vres =  _mm512_roundscale_ps(_mm512_mul_ps(vdata,vebx2) \
	...
	for (; id < blk_end8; id += 8) //AVX-256
	{
		...
		//perform mulitply and round
		const __m256 vres =  _mm256_round_ps(_mm256_mul_ps(vdata,veb_256) \
	                                            ,(_MM_FROUND_TO_NEAREST_INT |_MM_FROUND_NO_EXC));
	...
	for (; id < blk_end4; id += 4) //AVX-128
	{
		...
		//perform mulitply and round
		const __m128 vres =  _mm_round_ps(_mm_mul_ps(vdata,veb_128) \
	                                            ,(_MM_FROUND_TO_NEAREST_INT |_MM_FROUND_NO_EXC));
